...
entity HDMI_AXI_Master is
		...
		-- Width of User Response Bus
		  ...
		  -- Nombre de burst et adresse sont reçu par le composant HDMI_AXI_Slave
		  -- Car c'est le HDMI_AXI_Slave qui gère les registres de configuration du HDMI.
...
architecture implementation of HDMI_AXI_Master is
...
begin
	...
	--manner as the write address channel.
		  ...
		  begin
					 if rising_edge (M_AXI_ACLK) then
								if (M_AXI_ARESETN = '0') or (master_start_read = '1') then
										  axi_arvalid <= '0';
	...
	-- Next address after ARREADY indicates previous address acceptance
		  ...
		  begin
					 if rising_edge (M_AXI_ACLK) then
								if (M_AXI_ARESETN = '0') or (master_start_read = '1') then
										  axi_araddr <= master_base_address;
								else
	...
	-- terminal count to reduce decode logic
		  ...
		  begin
					 if rising_edge (M_AXI_ACLK) then
								if (M_AXI_ARESETN = '0') or (start_single_burst_read = '1') or (master_start_read = '1') then
										  read_index <= (others => '0');
								else
	...
	-- Register and hold read/write interface errors
		  ...
		  begin
					 if rising_edge (M_AXI_ACLK) then
								if (M_AXI_ARESETN = '1') and (master_start_read = '0') then
	...
	-- against the number of burst transactions the master needs to initiate
		  ...
		  begin
					 if rising_edge (M_AXI_ACLK) then
								if (M_AXI_ARESETN = '0') or (master_start_read = '1') then
										  read_burst_counter <= master_burst_nb;
								else
										  if (M_AXI_ARREADY = '1') and (axi_arvalid = '1') then
	...
	----------------------------------
		  ...
		  begin
				...
				-- state transition
										  case mst_exec_state is
													 ...
													 when IDLE =>
																master_sensor(31 downto 30) <= "11";
						-- This state is responsible to initiate
						-- AXI transaction when master_start_read is asserted
																if master_start_read = '1' then
																		  if unsigned(master_burst_nb) > 0 then
						...
						-- Read transactions will be issued until burst_read_active signal is asserted.
							...
							-- Launch read bursts only when the FIFO is empty enough
																		  if axi_arvalid = '0' and burst_read_active = '0' and prog_full = '0' and
								...
								-- Immediately clear to generate a pulse
																...
																master_sensor(31 downto 27) <= "01" & axi_arvalid & burst_read_active & start_single_burst_read;
		  ...
		  -- Signaux de debug.
		  master_sensor(16) <= '1' when (M_AXI_ARESETN = '1') and (master_start_read = '0') and (rnext = '1') else '0';
		  master_sensor(17) <= prog_full;
		  master_sensor(18) <= rnext;
		  master_sensor(19) <= M_AXI_RLAST;

		  
		  master_busy <= '1' when mst_exec_state /= IDLE else '0';

		  fifo_din <= M_AXI_RDATA when rnext = '1' else (others => '0');

		  fifo_wr_en <= '1' when (M_AXI_ARESETN = '1') and (master_start_read = '0') and (rnext = '1') else '0';

		  -- Signal d'écriture car le master a aussi (en théorie) la possibilité d'écrire.
		  axi_wdata <= (others => '0');
end implementation;
